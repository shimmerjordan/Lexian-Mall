				...
				<block v-for="(item, index) in cartList" :key="item.id">
					<view class="cart-item" :class="{'b-b': index!==cartList.length-1}">
						...
						<view class="item-right">
							...
							<uni-number-box class="step" :min="1" :max="item.storage" :value="item.commodity_quantity>item.storage?item.storage:item.commodity_quantity"
							 :isMax="item.commodity_quantity>=item.storage?true:false" :isMin="item.commodity_quantity===1" :index="index"
					...
					success: (e) => {
						if (e.confirm) {
							...
							for (var i = 0; i < this.cartList.length; i++) {
								this.cartIdList.push(this.cartList[i].cartID);
							}
							console.log(this.cartIdList);
							uni.request({
